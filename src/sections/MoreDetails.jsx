...
const MoreDetails = () => {
  return (
    <div className="flex justify-center items-end w-full min-h-screen py-12">
      <div className="w-[1107px] h-[717px] bg-white shadow-lg p-6 rounded-2xl">
        <div className="text-center mb-8">
          <h6 className="text-lg font-semibold text-gray-600">More Details</h6>
          <h2 className="text-3xl font-bold text-[#0A2C8C]">Get Your Answers</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.map((item, index) => (
            <div 
              key={index} 
              className={`${index === 0 ? 'bg-[#03135B] text-[#fff]' : 'bg-white text-black'} p-6 rounded-xl shadow-md`}
            >
              <img
                src={item.img} 
                alt="Placeholder"
              ...
              <h4 className="text-md font-semibold text-[#0A2C8C]">{item.subtitle}</h4>
              <p className="mt-2">
                 {item.description}
              </p>
            </div>
